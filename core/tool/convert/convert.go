...
func ToKindValue(src reflect.Value, kind reflect.Kind) (result reflect.Value, err error) {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		...
		if err != nil {
			err = fmt.Errorf("convert src to the kind failed: %w", err)
		}
	...
	default:
		err = fmt.Errorf("convert src to the kind failed: unsupported kind <%s>", src.Kind().String())
		return
	}
...
func BoolTo(src reflect.Value, kind reflect.Kind) (result reflect.Value, err error) {
	...
	default:
		err = fmt.Errorf("unsupported kind <%s>", kind.String())
	}
